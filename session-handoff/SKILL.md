...
1. Never start a competing GPU-heavy LLM directly when the supervisor can schedule it.
...
6. Use an ephemeral inquiry fork for a question that should **not** change the worker's canonical history.
7. Do not edit Hermes' internal `state.db` directly.
8. Use the configured Hermes adapter. For Hermes 0.19.x, use `hermes chat --resume ... --query ... --quiet`; use REST only when the installed build exposes and verifies the Sessions API.
...
## Ask without polluting worker context

Use this for retrospective questions, explanations, lookup, or clarification that should leave the canonical worker session unchanged:

```text
python "<HANDOFFCTL>" inquire worker-auth "Why did you choose token rotation every 15 minutes?" --detached --resume-origin
```

The supervisor should fork the canonical worker session, ask in the fork, capture the answer, and delete/archive the scratch fork.

## Run an external GPU-conflicting task
